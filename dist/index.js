...
app.post('/calcular', async (ctx) => {
    const { metodo, potencia, voltagem, temperaturaAmbiente, numeroCircuitos, proximoCabo, } = (await ctx.req.json());
    try {
        ...
        if (!cabo) {
            throw new Error('Nenhum cabo nominal encontrado para a corrente fornecida.');
        }
        const fatorTemperatura = getFatorCorrecaoTemperaturaByTemperaturaAmbiente(temperaturaAmbiente);
        if (!fatorTemperatura) {
            throw new Error('Nenhum fator de correção encontrado para a temperatura ambiente fornecida.');
        }
        ...
        const fatorCorrecaoFinal = calculaFatorCorrecaoFinal(fatorTemperatura.pvc, fatorAgrupamento.fator);
        const capacidadeAjustada = calculaCapacidadeAjustada(fatorCorrecaoFinal, cabo.a);
        ...
        const response = {
            ...
            fatorCorrecaoFinal,
            recomendacoes: {
                disjuntor: dijuntor || 'Nenhum disjuntor encontrado: mude para o próximo cabo.',
                mensagemCabo: capacidadeAjustada >= amperagem
